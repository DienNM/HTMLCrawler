...
public final class WorkerItemValidator {
    ...
    public static void validateDetailType(WorkerItemData workerItem, List<String> errors) {
        if (StringUtils.isEmpty(workerItem.getItemKey())) {
            errors.add(String.format("%s [CrawlType = %s] is missing Item", workerItem.getLevel(),
                    workerItem.getClass()));
        }
    ...
    public static void validateListType(WorkerItemData workerItem, List<String> errors) {
        if (workerItem.getPagingConfig() == null) {
            errors.add(String.format("%s [CrawlType = %s] is missing Paging Information", workerItem.getLevel(),
                    workerItem.getClass()));
        }

        AttributeSelector selector = workerItem.getLevel0Selector();
        if (selector == null || StringUtils.isEmpty(selector.getSelector())) {
            errors.add(String.format("%s [CrawlType = %s] is missing Link Selector", workerItem.getLevel(),
                    workerItem.getClass()));
        }
    }
}
